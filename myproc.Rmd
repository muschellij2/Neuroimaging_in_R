...
```{r fslsmooth, results='hide', echo = FALSE, message = FALSE}
...
double_ortho(base_t1, robust_window(n4_t1), NA.x = TRUE, add.orient = FALSE, crosshairs = FALSE)
```
...
ortho2(robust_window(n4_t1), ratio, col.y = alpha(hotmetal(), 0.75), add.orient = FALSE, crosshairs = FALSE)
```
...
- `extrantsr::registration` (wraps `ANTsR::antsRegistration`)

<img src="Reg_Figure_Option1_short_within2.png" style="width:60%">
...
q = qplot(t1, flair, data = s, geom = "bin2d", bins = 100) + 
  ...
  theme_grey(base_size = 25) + labs(x = "T1 Intensity", y = "FLAIR Intensity") 
